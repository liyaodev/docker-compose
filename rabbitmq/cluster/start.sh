...
docker-compose up -d
sleep 15
docker exec -ti builder-rabbitmq2 bash -c "$JOIN_RABBIT2_RABBIT1"
docker exec -ti builder-rabbitmq3 bash -c "$JOIN_RABBIT3_RABBIT1"
docker exec -ti builder-rabbitmq1 bash -c "$OPTIONAL_COMMAND"
